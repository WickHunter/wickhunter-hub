...
- v0.4.28 — **The admin deletion hold is real, and the provider cost ceiling refuses a checkout before it makes a promise it cannot keep.** Both were named as gaps in v0.4.27's own changelog entry.
  ...
  * **Cost ceiling**: `HostingInstanceRow.providerPlanMonthlyCostCents` (new field) is captured once per instance from `provider.listPlans()` the first time it is provisioned — best-effort (a failed read just leaves it `null`, never blocks provisioning itself), never guessed. `HostingService.projectedMonthlyProviderCostCents()` sums it across every non-deleted instance (suspended/past_due/cancel_scheduled/restoring included on purpose — a restore creates a provider resource again, so it counts against budget even while powered off); ANY non-deleted instance with an uncaptured quote makes the WHOLE projection `{known: false}`, never silently 0 (a fabricated zero would understate spend and let the ceiling pass when it should refuse). `checkoutUrl` now refuses by name ("hosting is at its provider cost ceiling") before minting a payment link whenever a non-zero ceiling is configured and admitting one more instance — using the plan's own most-recently-recorded quote from any instance of the same plan, no live provider call — would push the projection over it; the customer gets an honest refusal rather than a payment prompt provisioning would later refuse. 0 = no ceiling, unchanged existing semantics. The admin Hosting page's meta line now reads projected-vs-ceiling (or "unknown … new checkouts are refused" when a quote is missing), and `GET /admin/api/hosting/policy` carries `projectedMonthlyProviderCostCents` alongside the policy it is measured against.
  * `tests/hosting-holds.test.mjs` (new, registered by `run-all.mjs`'s own directory scan — no separate registration step in this repo): hold blocks the delete transition across five ticks well past the deadline; force-delete on a held instance is refused by name; release recomputes from the original instant plus held-duration (mutation-verified: anchoring on the release clock instead turns the test red); release past an already-elapsed recomputed deadline proceeds on the next tick with exactly one terminated email and zero stale reminders; a second hold call never moves the hold's own `atMs`; the cost ceiling passes exactly at the boundary and refuses one cent over (FakeProvider's `createCalls` proves nothing was attempted for the refused checkout); an uncaptured quote refuses even under a ceiling that would obviously not be exceeded; a zero ceiling never refuses; the admin policy route reports the projection; the two HTTP routes (admin-token gated, 404/409 on the same shapes every other hosting admin route uses).
  * Not live-proved (unchanged from v0.4.27): Vultr field mappings, including `listPlans()`'s real response shape — the cost-ceiling capture is only as good as that mapping until a real account exercises it.
- v0.4.27 — **Hosted VPS provisioning (H4/H5/H6), shipped OFF.** `src/hosting/`: a file-backed transactional store (`hosting-db.v1.json`: instances, provider resources, billing inbox, outbox — one synchronous `mutate()` write per transition so an instance change and its outbox job are one atomic write; CAS versions, TTL leases; one active instance per owner), `HostingProvider` interface with `VultrProvider` (doc-read, never exercised by tests) and `FakeProvider`, the order row + deterministic label persisted BEFORE any provider call and `findByLabel` checked before any retry creates; `policy.ts` defaults from the app repo's `docs/HOSTING-REGION-2026-09-10.md` (regions `nrt` then `itm`, plan `vc2-1c-2gb`, price $15 labelled proposed, seven-venue reachability probe list; `provisioningEnabled` false until `osId` + `releaseRef` are set); `deadlines.ts` pure UTC policy (T+72h suspend / T+240h delete on nonpayment, T / T+168h on voluntary cancel, 3d/1d reminders); `HostingService` derives every transition from the hosting-role billing record (v0.4.25) and a periodic `tick()`, restore always lands on `ready` (paused) through the same readiness gate as a fresh box, readiness refused by venue name fails over to the fallback region once, a late payment on a deleted instance never resurrects it, a hosting refund never touches the software customer record; routes `GET /api/hosting/options`, `GET /api/hosting`, `POST /api/hosting/checkout`, `POST /api/hosting/:id/cancel|resume-renewal`, `POST /api/hosting/instances/:id/readiness` (bootstrap-token auth), admin policy/instances/retry/force-suspend/force-delete; customer hosting card and admin Hosting tab; nine email templates. Not live-proved: Vultr field mappings, Stripe cancel/resume/refund calls; the checkout attaches to the existing customer by prefilled email only; the admin deletion hold is a 501 stub; the cost ceiling is stored, not enforced.
- v0.4.26 — **The central customer dashboard (H2).** `src/customer-sessions.ts`: one identity per case-folded email, created only after a matching billing customer exists (`data/customer-identities.v1.json`); magic-link sign-in (`POST /api/customer/signin`, always `{ok:true}`, 5 per email per 15 min plus the shared IP limiter; single-use 15-minute tokens hashed at rest; `GET /customer/signin?token=` burns it and sets an HttpOnly/Secure/SameSite=Lax 30-day sliding session cookie, hashed at rest, restart-safe); `/customer` dashboard (`public/customer.html`) reading only hub state — licence id/plan/expiry/exit-only per software record with test and live listed separately, install command via the existing install token, Stripe portal link (which still exposes every subscription on the customer until per-role portal configs exist), hosting placeholder, sign-out; `POST /admin/api/customers/signin-link` for a bounced email. Customer cookies cannot reach `/admin/api/*` (header-only admin auth) and an admin token cannot read customer state — both pinned. **Repo fix:** v0.4.25 accidentally committed a `node_modules` symlink (the ignore pattern `node_modules/` matches directories only); untracked, and the pattern widened. `tests/customer-sessions.test.mjs`.
- v0.4.25 — **Billing events are classified software vs. hosting before any licence handler runs (H1).** `src/billing/roles.ts` + the dispatcher in `service.ts`: per-mode (test/live) Stripe price/product-id allowlists in `BillingConfig.roles`, `Plan.role` (defaults `software`) as the checkout binding aid, an object-id → role index (`data/billing-role-index.v1.json`) so a refund/dispute naming only a charge resolves without re-deciding, per-`(customer, role)` rows for non-software products (`data/billing-role-subscriptions.v1.json`), one-shot marker-guarded migration folding every existing customer into `software`. While the hosting allowlist is empty everything classifies `software`, byte-for-byte as before; an id on neither list is `unclassified` — recorded, applied to no role. Reproduced first: a second subscription on the same Stripe customer overwrote the software subscription id, moved its period, and a refund of the other product revoked the licence. `tests/billing-roles.test.mjs`.
- v0.4.24 — The setup video ("Install Wick Hunter Unleashed on Vultr | Welcome Email to First Login") is linked from the welcome email (secondary button + text) and from the install page above the command. One hardcoded URL, `SETUP_VIDEO_URL` in `src/billing/email.ts`.
